...
library(ineq)
vermoegen <- read.dta("data/vermoegen20131023.dta")
lorenz_vermoegen <- Lc(vermoegen$rverm[vermoegen$kanton=="CH"&vermoegen$jahr==2010],vermoegen$anz_pflichtige[vermoegen$kanton=="CH"&vermoegen$jahr==2010])
einkommen <- subset(read.dta("data/steuerdaten20131024.dta"), kanton=="CH"&steuerperiode==2010)

lorenz_vermoegen <- Lc(vermoegen$rverm[vermoegen$kanton=="CH"&vermoegen$jahr==2010],vermoegen$anz_pflichtige[vermoegen$kanton=="CH"&vermoegen$jahr==2010])

lorenz_einkommen  <- Lc(einkommen$steink,einkommen$anz_pflichtige)
...
ggplot(lorenzdata, aes(x=p,y=L,colour=Lorenzkurve)) + geom_line(size=2) + geom_segment(y=0, x=0,yend=1,xend=1,colour="black")+theme_bw()+scale_colour_grey(name="")+  geom_segment(y=0, x=0,yend=0,xend=1,colour="black")+  geom_segment(y=0, x=1,yend=1,xend=1,colour="black")+annotate("text",label="Gleichverteilung",x=0.55,y=0.6,angle=36,size=3)+annotate("text",label="Gini=0.79",x=0.7,y=0.27,angle=36,size=3)+annotate("text",label="Gini=0.36",x=0.6,y=0.47,angle=36,size=3)+ylab("Anteil am Gesamten")+xlab("Bevölkerung")
```
