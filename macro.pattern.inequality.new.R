...
summary(model.lag.1)
dwtest(model.lag)
model.lag.2<-dyn$lm(ts(Gini.g)~1+ts(sozialausgaben.g)+ts(mdp.g)+ts(foreigner.g)+ts(altersquotient_2.g)+lag(ts(Gini.g),-1)+ts(HHp1.g)+ts(uniondensity.g)+ts(sector2.g)+ts(sector3.g),macro)
summary(model.lag.2)
dwtest(model.lag.2)
model.lag.3<-dyn$lm(ts(Gini.g)~1+ts(sozialausgaben.g)+ts(mdp.g)+ts(foreigner.g)+ts(altersquotient_2.g)+lag(ts(Gini.g),-1)+ts(HHp1.g)+ts(uniondensity.g)+ts(sector1.g)+ts(sector3.g),macro)
summary(model.lag.3)
...
boot2 <- boot.relimp(model.lag.2, b = 1000, type = "lmg", rank = TRUE)
boot3 <- boot.relimp(model.lag.3, b = 1000, type = "lmg", rank = TRUE)
#booteval.relimp(boot) # print result
# difficult to extract from boot object
namen <- c("Sozialausgaben","GDP","Ausl�nderanteil","Altersquotient","Gini t-1","Unerkl�rte Varianz")
namen2 <- c("Sozialausgaben","GDP","Ausl�nderanteil","Altersquotient","Gini t-1","Einp.haushalte","Gewerkschaften","Sektor 2","Sektor 3","Unerkl�rte Varianz")
namen3 <- c("Sozialausgaben","GDP","Ausl�nderanteil","Altersquotient","Gini t-1","Einp.haushalte","Gewerkschafen","Sektor 1","Sektor 3","Unerkl�rte Varianz")
betas <- boot@boot$t0[4:8]
betas <-c(betas,1-sum(betas))
betas2 <- boot2@boot$t0[4:12]
betas2 <-c(betas2,1-sum(betas2))
betas3 <- boot3@boot$t0[4:12]
betas3 <-c(betas3,1-sum(betas3))
order <-1:6
order2 <-1:10
order3 <-1:10
bcolour<-c(rep("darkgrey",5),"grey")
bcolour2<-c(rep("darkgrey",9),"grey")
bcolour3<-c(rep("darkgrey",9),"grey")
min_ci <- c(sapply(data.frame(boot@boot$t[,4:8]),quantile,0.025),NA)
max_ci <- c(sapply(data.frame(boot@boot$t[,4:8]),quantile,0.975),NA)
min_ci2 <- c(sapply(data.frame(boot2@boot$t[,4:12]),quantile,0.025),NA)
max_ci2 <- c(sapply(data.frame(boot2@boot$t[,4:12]),quantile,0.975),NA)
min_ci3 <- c(sapply(data.frame(boot3@boot$t[,4:12]),quantile,0.025),NA)
max_ci3 <- c(sapply(data.frame(boot3@boot$t[,4:12]),quantile,0.975),NA)
bdata<-data.frame(betas,namen,min_ci,max_ci,order,bcolour,group=1)
bdata2<-data.frame(betas=betas2,namen=namen2,min_ci=min_ci2,min_ci=max_ci2,order=order2,bcolour=bcolour2,group=2)
bdata3<-data.frame(betas=betas3,namen=namen3,min_ci=min_ci3,max_ci=max_ci3,order=order3,bcolour=bcolour3,group=3)
bothdata <- rbind(bdata,bdata3)
library(ggplot2)
#plot(booteval.relimp(boot,sort=TRUE)) # plot result 
#summary(model.lag.1)
#summary(model.lag.2)
p1<-ggplot(bdata, width=10,aes(x=reorder(namen,order),y=betas,ymax=max_ci,ymin=min_ci))+geom_bar(stat="identity",fill=bcolour)+ geom_linerange(colour="black")+theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1))+scale_colour_grey()+xlab("R� = 0.42") +ylab("% erkl�rte Varianz") + scale_y_continuous(limits=c(0, 0.6))
#p2<-ggplot(bdata2, aes(x=reorder(namen2,order2),y=betas2,ymax=max_ci2,ymin=min_ci2))+geom_bar(stat="identity",fill=bcolour2)+ geom_linerange(colour="black")+theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1))+scale_colour_grey()+xlab("R� = 0.47") +ylab("% erkl�rte Varianz") + scale_y_continuous(limits=c(0, 0.6))
p3<-ggplot(bdata3, width=6,aes(x=reorder(namen3,order3),y=betas3,ymax=max_ci3,ymin=min_ci3))+geom_bar(stat="identity",fill=bcolour3)+ geom_linerange(colour="black")+theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1))+scale_colour_grey()+xlab("R� = 0.47")+ylab("") + scale_y_continuous(limits=c(0, 0.6))
#bothdata$min_ci<-ifelse(is.na(bothdata$min_ci),bothdata$betas,bothdata$min_ci)
#bothdata$max_ci<-ifelse(is.na(bothdata$max_ci),bothdata$betas,bothdata$max_ci)
#ggplot(bothdata, aes(x=reorder(namen,order),y=betas,ymax=max_ci,ymin=min_ci))+geom_bar(stat="identity")+ geom_linerange(colour="black")+theme_bw()+theme(axis.text.x = element_text(angle = 45, hjust = 1))+scale_colour_grey()+xlab("R� = 0.47") +ylab("% erkl�rte Varianz") + facet_grid(~group)
library(gridExtra)
grid.arrange(p1, p3, nrow=1,widths=c(7.5,11.2))
